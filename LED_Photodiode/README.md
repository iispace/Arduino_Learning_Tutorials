...
- LED가 만들어내는 광전류는 매우 낮은 수준이므로, 이 신호를 증폭해 주는 TIA 소자(예 LTC1050나 OPA657 같은 입력 바이어스 전류가 매우 작은 타입의 광검출기(포토다이오드)용 트랜스임피던스 증폭기(TIA))와 커패시터 등을 회로에 추가하면 더 정밀한 측정값을 얻을 수 있다고...
