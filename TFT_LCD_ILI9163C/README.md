...
# header 파일 수정 
(1) 본 제품은 PCB 색상이 검정색인 제품과 빨간색인 제품이 있고, PCB의 색상에 따라 설정된 GRAM의 OFFSET이 다르다. 그런데, 간혹 빨간색 PCB 제품인데도 검정색 PCB 제품의 OFFSET 설정을 해 주어야 하는 제품도 있는데, 테스트 프로그램을 업로드하여 실행해 보면 지금 사용하고 있는 제품의 설정에 수정이 필요한 지 그렇지 않은 지를 확인할 수 있다[[1]]([1]). 여기서 GRAM이란 Graphic RAM으로, TFT LCD 컨트롤러 칩 내부에 있는 메모리를 말한다. ILI9163C 컨트롤러 칩을 탑재한 LCD의 경우, 칩 자체는 160x128의 해상도를 지원하도록 설계되었지만, 실제 물리적인 LCD 패널은 그 보다 작은 128x128 해상도를 지원하기 때문에 OFFSET 설정이 맞지 않는 경우에는 아래의 왼쪽 사진(비정상)과 같이 화면의 일부를 사용할 수 없게 되고, 해당 영역은 노이즈가 출력되는 것처럼 보인다.
